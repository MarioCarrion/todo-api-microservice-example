...
func NewMessageHub(conf *envvar.Configuration, logger *zap.Logger) (MessageBus, error) {
...
func (n *noOpRepository) Created(ctx context.Context, task internal.Task) error {
	n.logger.Info("noop created", zap.String("task", fmt.Sprintf("%+v", task)))
	return nil
}

func (n *noOpRepository) Deleted(ctx context.Context, id string) error {
	n.logger.Info("noop deleted", zap.String("task", id))
	return nil
}

func (n *noOpRepository) Updated(ctx context.Context, task internal.Task) error {
